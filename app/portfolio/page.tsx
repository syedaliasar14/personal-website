      ...
      <main className="flex flex-col w-full md:max-w-5xl items-center px-4 flex-grow">
        ...
        <div className="relative flex flex-col w-full gap-16 ">
          {projects.map((project) => (
            <Link href={project.link || "#"} target="_blank" key={project.name} className="relative group flex flex-col w-full items-center rounded-lg overflow-hidden border border-slate-800 hover:border-green-300 transition-colors duration-300 ease-in-out">
              <div className={`absolute top-0 relative flex justify-center overflow-hidden flex w-full h-[220px] md:min-h-[400px]`}>
                <Image className="absolute object-cover group-hover:scale-105 transition-transform duration-300" src={`/portfolio/${project.slug}/1.png`} alt={project.name} fill sizes="" />
              </div>
